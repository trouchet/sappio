import log, { raise } from './logger.js';
...
export const convertTimeDouble = (value, from_unit_str, to_unit_str) => {
  if (typeof value !== 'number' || value < 0) {
    raise('Value must be a positive number!');
    return;
  }

  if (typeof from_unit_str !== 'string' || typeof to_unit_str !== 'string') {
    raise(`From and to unit strings must belong to list [ ${timeHierarchy} ]`);
    return;
  }
  ...
  if (fromIndex === -1) {
    raise(errorMsg.replace('_ITEM_', from_unit_str));
  } else if (toIndex === -1) {
    raise(errorMsg.replace('_ITEM_', to_unit_str));
  } else {
    let reduce_fun = [];
